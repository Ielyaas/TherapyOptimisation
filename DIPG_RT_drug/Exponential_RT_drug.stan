...
// dP/dt = gamma*P - (beta_q + beta_a)*P, 
// dQ/dt = beta_q*P - beta_a*Q,
// dD/dt = beta_a*P - delta*D.
// Tumour growth modelled as Exponential growth and decay of senescent
// and dying cells based on DIPG7 data from 
...
functions {
  ...
  real[] cell_population_dynamics(real t,		              // Time at which derivatives are evaluated
    ...
    real beta_q = params[2];                              // Rate of entry into senescence
    real beta_a = params[3];                              // Rate of entry into apoptosis
    real delta = params[4];                               // Rate of removal of dead cells
    
    real dN_dt[3];

    dN_dt[1] = gamma*N[1] - (beta_q + beta_a)*N[1];       // Proliferating cells
  ...
  real[] control_tumor_volume_dynamics(real[] times, real t0, real V_t0,
                                       real gamma, real beta_q, real beta_a, real delta,
                                       real[] x_r, int[] x_i,
                                       real rel_tol, real abs_tol, int max_num_steps) {
    
    real N_t0[3];                                         // Initial state
    real params[4];                                       // ODE parameters
  ...
  real[] drug_rt_tumor_volume_dynamics(real[] times, real[] tx_times, real[] tx_doses,
                                       ...
                                       real V_t0, real gamma, real beta_q, real beta_a,
...
parameters {
  ...
  corr_matrix[5] rho;                                     // Correlation matrix
  vector<lower = 0>[5] omega;                             // Standard deviations for inter-individual variability
  real<lower = 0> sigma;                                  // Residual standard deviation
  vector[5] logtheta[n_mice];                             // Individual level parameter values
  
}

transformed parameters {

  vector<lower = 0>[5] theta_hat;                         // Population level means of parameter values
  ...
  theta_hat[2] = beta_q_hat;                              // Popultaion level mean beta_quiescent
  theta_hat[3] = beta_a_hat;                              // Popultaion level mean beta_apoptotic
  theta_hat[4] = delta_hat;                               // Popultaion level mean death rate
  theta_hat[5] = alpha_hat;                               // Popultaion level mean alpha
  ...
  for (i in 1:n_mice) {
    ...
    beta_q[i] = exp(logtheta[i, 2]);
    beta_a[i] = exp(logtheta[i, 3]);
    delta[i] = exp(logtheta[i, 4]);
    alpha[i] = exp(logtheta[i, 5]);

    if (i <= n_control_mice) {
      V_hat[start[i]:end[i]] =
        control_tumor_volume_dynamics(times[start[i]:end[i]], t0[i], V_t0[i],
                                      gamma[i], beta_q[i], beta_a[i], delta[i],
                                      x_r, x_i,
                                      rel_tol, abs_tol, max_num_steps);
    } else {
      V_hat[start[i]:end[i]] =
        drug_rt_tumor_volume_dynamics(times[start[i]:end[i]],
                                      ...
                                      gamma[i], beta_q[i], beta_a[i], delta[i], alpha[i],
                                      x_r, x_i,
                                      rel_tol, abs_tol, max_num_steps); 
    }
  }
...
model {
  ...
  gamma_hat ~ normal(1.5, 1);
  // beta_q_hat ~ normal(0.01, 1);
  beta_q_hat ~ normal(0.2, 1);
  // beta_a_hat ~ normal(0.01, 1);
  beta_a_hat ~ normal(1.1, 1);
...
generated quantities {
  ...
  for (i in 1:n_mice) {
    ...
    beta_q_pred[i] = exp(logtheta_pred[i, 2]);
    beta_a_pred[i] = exp(logtheta_pred[i, 3]);
    delta_pred[i] = exp(logtheta_pred[i, 4]);
    alpha_pred[i] = exp(logtheta_pred[i, 5]);
    
    if (i <= n_control_mice) {
      V_hat_pred_times[start[i]:end[i]] =
        control_tumor_volume_dynamics(times[start[i]:end[i]], t0[i], V_t0[i],
                                      gamma_pred[i], beta_q_pred[i], beta_a_pred[i],
