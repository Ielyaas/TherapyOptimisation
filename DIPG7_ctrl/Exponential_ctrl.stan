...
// dP/dt = gamma*P.
// Tumour growth modelled as Exponential growth based on DIPG7 data from 
...
functions {
  ...
  real[] cell_population_dynamics(real[] times,	  // Times at which system dynamics are evaluated
    ...
    for (t in 1:size(times)) {
      ...
      N[t] = N0*exp((gamma)*(times[t] - times[1])); // proliferating compartment dynamics
    }
