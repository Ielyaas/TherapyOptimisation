...
// dP/dt = gamma*P(1 - P/K).
// Tumour growth modelled as Logistic growth based on DIPG7 data from 
...
functions {
  ...
  real[] cell_population_dynamics(real[] times,	  // Times at which system dynamics are evaluated
    ...
    for (t in 1:size(times)) {
      // Logistic
      N[t] = k*N0*exp(gamma*(times[t] - times[1]))/(k + N0*((exp(gamma*(times[t] - times[1]))) - 1));
  ...
  real[] control_tumor_volume_dynamics(real[] times, real V_t0,
    ...
    N_hat = cell_population_dynamics(times, N_t0, params);
...
model {
  ...
  // Priors
  // Note that priors are also informed by parameter contraints
