...
// senescent cells (Q).
// dP/dt = gamma*P-delta*P, dQ/dt = delta*P.
...
functions {
  ...
  real[,] cell_population_dynamics_num(real[] times,	  // Times at which system dynamics are evaluated
    ...
    real delta = params[2];                                 // Senescent rate
    ...
    for (t in 1:size(times)) {
      ...
      N[t,2] = N0[1]/(gamma - delta)*(exp((gamma-delta)*(times[t] - times[1])) - 1) + N0[2]; // senescent compartment dynamics
    }
  ...
  real[] control_tumor_volume_dynamics(real[] times, real V_t0,
    ...
    matrix[size(times), 2] N_hat;                         // Predicted cell population counts
  ...
  real[] tumor_volume_dynamics(real[] times, real[] rt_times, real[] rt_doses, int[] fraction_index, real V_t0,
                               real gamma, real delta, real alpha,
                               real[] x_r, int[] x_i) {
    ...
    N_t0[1] = N_hat[fraction_index[size(fraction_index)], 1]*exp(-alpha*rt_doses[size(rt_times)]);
    N_t0[2] = N_hat[fraction_index[size(fraction_index)], 2] +
      N_hat[fraction_index[size(fraction_index)], 1]*exp(-alpha*rt_doses[size(fraction_index)]);
...
parameters {
  ...
  real logdelta_hat;                                  // rate of continous transfer from proliferating to senescent compartment
...
transformed parameters {
  ...
  log_theta_hat[2] = logdelta_hat;                                   // Populataion level mean carrying capacity
      ...
      V_hat[start[i]:end[i]] =
        tumor_volume_dynamics(times[start[i]:end[i]],
                              ...
                              fraction_index[schedule_start[i - n_untreated_mice]:schedule_end[i - n_untreated_mice]],
                               V_t0[i],
                              gamma[i], delta[i], alpha[i], x_r, x_i);
