...
// senescent cells (Q) and dying tumour population (A).
// dP/dt = gamma*P*log(K/P), dQ/dt = 0, dA/dt = delta*P.
...
functions {
  ...
  real[,] cell_population_dynamics_num(real[] times,	  // Times at which system dynamics are evaluated
    ...
    for (t in 1:size(times)) {
      // model 17
      N[t,1] = N0[1]*exp((gamma/K)*(1 - exp(-K*(times[t] - times[1])/(gamma - delta/gamma))));
      N[t,2] = N0[2]*exp((gamma/K)*-(1 - exp(-K*(times[t] - times[1]))));
      N[t,3] = N0[3]*exp((gamma/K)*-(1 - exp(-K*(times[t] - times[1])/(gamma - delta/gamma))));
    }
  ...
  real[] tumor_volume_dynamics(real[] times, real[] rt_times, real[] rt_doses, int[] fraction_index, real V_t0,
                               real gamma, real K, real delta, real alpha1, real alpha2,
                               real[] x_r, int[] x_i) {
    ...
    N_t0[1] = N_hat[size(fraction_index), 1]*exp(-(alpha1 + alpha2)*rt_doses[size(fraction_index)]);
    N_t0[2] = N_hat[size(fraction_index), 2] +
      N_hat[size(fraction_index), 1]*exp(-alpha1*rt_doses[size(fraction_index)]);
    N_t0[3] = N_hat[size(fraction_index), 3] +
      N_hat[size(fraction_index), 1]*exp(-alpha2*rt_doses[size(fraction_index)]);
