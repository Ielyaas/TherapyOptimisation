...
// senescent cells (Q).Instantaneous RT effect
// dP/dt = gamma*P-delta*P, dQ/dt = 0.
...
functions {
  ...
  real[,] cell_population_dynamics_num(real[] times,	  // Times at which system dynamics are evaluated
    ...
    for (t in 1:size(times)) {
      ...
      N[t,1] = N0[1]*exp(gamma*(times[t] - times[1]));                      // proliferating compartment dynamics
...
model {
  ...
  logalpha_hat ~ normal(-2, 0.1);
  logV_t0_hat ~ normal(215.5, 0.1);
