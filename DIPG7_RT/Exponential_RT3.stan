// Stan code for DIPG7 tumour growth and response to RT. Single rate of 
// RT-dependent senescence and death.
...
functions {
  ...
  real[,] cell_population_dynamics_num(real[] times,	  // Times at which system dynamics are evaluated
    ...
    for (t in 1:size(times)) {
      ...
      N[t,3] = (delta*N0[1])/(gamma - delta)*(exp((gamma-delta)*(times[t] - times[1])) - 1) + N0[3]; // dying compartment dynamics
    }
