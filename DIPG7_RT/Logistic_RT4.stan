...
functions {
  ...
  real[] tumor_volume_dynamics(real[] times, real[] rt_times, real[] rt_doses, int[] fraction_index, real V_t0,
    ...
    N_t0[1] = N_hat[size(rt_times), 1]*exp(-(alpha1 + alpha2)*rt_doses[size(rt_times)]);
    N_t0[2] = N_hat[size(rt_times), 2] +
      N_hat[size(rt_times), 1]*exp(-alpha1*rt_doses[size(rt_times)]);
    N_t0[3] = N_hat[size(rt_times), 3] +
      N_hat[size(rt_times), 1]*exp(-alpha2*rt_doses[size(rt_times)]);  
...
data {
  ...
  int<lower = 0> fraction_index[n_fractions];                        
